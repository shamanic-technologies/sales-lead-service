    ...
    byCause: z
      ...
      .openapi({
        description:
          ...
          "have nothing to do with us. outreach — our outreach caused it. other — something else " +
          "of theirs did; the outcome is REAL, it stays in `counts` and among the brand's own, it " +
          "is simply not one to compute OUR return on. Each answer is a person's when one was " +
          "given, else the owner's date rule, whatever the source (after our first delivered email " +
          "to that person -> outreach, before -> other). unstated — UNDECIDED: undated, not " +
          "attributed to a lead we emailed, never delivered to, or not evaluated yet. `unstated` is " +
...
registry.registerPath({
  ...
  description:
    ...
    "other outcome whatever its source: after our first delivered email to that person -> outreach, " +
    "before -> other, undated / unmatched / never delivered -> unstated. outreach + other + unstated " +
    "=== counts, per key. A \"never\" " +
...
const ConvertedLeadOutcomeSchema = z
  .object({
    ...
    causedByOutreach: z.boolean().nullable().openapi({
      description:
        "WHOSE win it was. true — our outreach caused this outcome; false — something else of the " +
        "customer's did (a referral, a conference, their existing pipeline, another agency): the " +
        "outcome is REAL and is counted everywhere the others are, it is simply not one to compute " +
        "the return on our outreach from, so a consumer leaves its value out of that figure and keeps " +
        "it in the brand's own total. A person's answer when one was given, else the owner's date " +
        "rule (see causeBasis / causeReason) — whatever the source. null — UNDECIDED: undated, not " +
        "attributed to a lead we emailed, never delivered to, or not evaluated yet. Null is never read " +
...
const HistoryEventSchema = z
  .object({
    ...
    destination: z
      .object({
        state: z.enum(["known", "ambiguous", "unknown"]).openapi({
          description:
            "known: we can say where this click went. ambiguous: the copy we wrote on this campaign pointed at several places, so there is nowhere honest to point. unknown: nothing resolvable at all. The last two are different facts — \"there is nowhere to point\" is not \"we have no opinion\".",
        }),
        href: z.string().nullable().openapi({
          description:
            "The URL we wrote, tracking parameters included. Null unless `state` is `known` — never a guess between candidates, and NEVER the outreach provider's click-tracking redirect, which following from a dashboard would register a click the prospect never made.",
        }),
        resolution: z.enum(["observed", "deduced"]).nullable().openapi({
          description:
            "observed: a record of the destination itself. deduced: worked out from the copy we wrote to this person on this campaign, because the delivery layer measures THAT a click happened and never where it went. Null unless `state` is `known`.",
        }),
      })
      .optional()
      .openapi({
        description:
          "On a `delivery` event whose milestone is `clicked`: where that click went. Present on no other milestone — a history with no click is byte-identical to what it always was.",
      }),
