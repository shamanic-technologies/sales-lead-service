 ...
 *
 * Connect-phase retry, not a raw `fetch`: this is the serve path's own read of
 * brand-service, so a sibling mid-restart resetting the connection must not
 * fail a serve. Retries are write-safe (the request never reached the server)
 * and only fire on a thrown rejection — a non-2xx is a real answer and still
 * fails loud.
 */
